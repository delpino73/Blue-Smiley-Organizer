...
if (isset($_POST['save'])) {
	...
	$base_instance->query("UPDATE {$base_instance->entity['FILE']['MAIN']} SET category='$new_category' WHERE user='$userid' AND category=$category_id");

	$base_instance->query("DELETE FROM {$base_instance->entity['FILE']['CATEGORY']} WHERE user='$userid' AND ID='$category_id'");

	$base_instance->show_message('Categories have been merged','<a href="show-file-categories.php">[Show all Categories]</a>');
