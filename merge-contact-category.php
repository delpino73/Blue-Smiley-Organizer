...
if (isset($_POST['save'])) {
	...
	$base_instance->query("UPDATE {$base_instance->entity['CONTACT']['MAIN']} SET category='$new_category' WHERE user='$userid' AND category=$category_id");

	$base_instance->query("DELETE FROM {$base_instance->entity['CONTACT']['CATEGORY']} WHERE user='$userid' AND ID='$category_id'");
...
$data=$base_instance->get_data("SELECT * FROM {$base_instance->entity['CONTACT']['CATEGORY']} WHERE user='$userid'");
