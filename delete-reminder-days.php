...
if (isset($_POST['save'])) {
	...
	$base_instance->query("DELETE FROM {$base_instance->entity['REMINDER']['DAYS']} WHERE ID='$reminder_id' AND user='$userid'");

	header('Location: close-me.php'); exit;

}

$data=$base_instance->get_data("SELECT * FROM {$base_instance->entity['REMINDER']['DAYS']} WHERE ID='$reminder_id' AND user='$userid'");
