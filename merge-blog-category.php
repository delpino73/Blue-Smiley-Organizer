...
if (isset($_POST['save'])) {
	...
	$base_instance->query("UPDATE {$base_instance->entity['BLOG']['MAIN']} SET category='$new_category' WHERE user='$userid' AND category=$category_id");

	$base_instance->query("DELETE FROM {$base_instance->entity['BLOG']['CATEGORY']} WHERE user='$userid' AND ID='$category_id'");

	$base_instance->show_message('Categories have been merged','<a href="show-blog-categories.php">[Show all Categories]</a>');
