...
if (isset($_POST['save'])) {
	...
	$data=$base_instance->get_data("SELECT * FROM {$base_instance->entity['FILE']['MAIN']} WHERE user='$userid' AND category='$category_id'");

	for ($index=1; $index <= sizeof($data); $index++) {

	$filename=$data[$index]->filename;

	if (file_exists('./upload/'.$userid.'/'.$filename)) { unlink('./upload/'.$userid.'/'.$filename); }

	}

	$base_instance->query("DELETE FROM {$base_instance->entity['FILE']['MAIN']} WHERE user='$userid' AND category='$category_id'");

	$base_instance->query("DELETE FROM {$base_instance->entity['FILE']['CATEGORY']} WHERE user='$userid' AND ID='$category_id'");
