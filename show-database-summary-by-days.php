...
$period=date('Y-m-d',mktime(0,0,0,date('m'),date('d')-$days,date('Y')));
...
for ($index=1; $index <= sizeof($data); $index++) {
	...
	$data_total=$base_instance->get_data("SELECT COUNT(*) AS total FROM {$base_instance->entity['DATABASE']['SELECT_VALUES']} WHERE category_id='$category_id' AND user='$userid' AND select_field_id='$ID' AND date > '$period'");
	...
	$data2=$base_instance->get_data("SELECT value, count(value) as cnt FROM {$base_instance->entity['DATABASE']['SELECT_VALUES']} WHERE category_id='$category_id' AND user='$userid' AND select_field_id='$ID' AND date > '$period' GROUP BY value");
...
for ($index=1; $index <= sizeof($data); $index++) {
	...
	$data_total=$base_instance->get_data("SELECT COUNT(*) AS total FROM {$base_instance->entity['DATABASE']['CHECKBOX_VALUES']} WHERE category_id='$category_id' AND user='$userid' AND checkbox_field_id='$ID' AND date > '$period'");

	$total_number=$data_total[1]->total;

	$text.='<table border=1 cellspacing=0 cellpadding=10 bgcolor="#ffffff" class="pastel"><tr><td><table border=1 cellspacing=0 cellpadding=2 bgcolor="#ffffff" class="pastel"><tr><td bgcolor="#dedede"><b>Field Name:</b></td><td>'.$title.'</td></tr><tr><td bgcolor="#dedede"><b>Total Number:</b></td><td>'.$total_number.'</td></tr></table>';

	$data2=$base_instance->get_data("SELECT value, count(value) as cnt FROM {$base_instance->entity['DATABASE']['CHECKBOX_VALUES']} WHERE category_id='$category_id' AND user='$userid' AND checkbox_field_id='$ID' AND date > '$period' GROUP BY value ORDER BY cnt DESC");
...
for ($index=1; $index <= sizeof($data); $index++) {
	...
	$data_total=$base_instance->get_data("SELECT COUNT(*) AS total, AVG(value) AS average, SUM(value) AS sum, MAX(value) AS max_value, MIN(value) AS min_value, STD(value) AS std FROM {$base_instance->entity['DATABASE']['NUMBER_VALUES']} WHERE category_id='$category_id' AND number_field_id='$ID' AND date > '$period' AND user='$userid'");
...
array(
'HEADER'=>'Show Data (Last '.$days.' days)',
